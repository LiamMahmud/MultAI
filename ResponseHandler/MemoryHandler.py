...
class MemoryHandler:
    ...
    def needed_space(self):
        if self.model_config["model_type"] == "chat":
            ...
            number_layers = float(self.read_model_config(config_path, "number_layers"))
            model_size = float(self.read_model_config(config_path, "model_size"))
            opt_offload = int(self.optimal_offload(model_size, number_layers))
            ...
            if "n_gpu_layers" in self.model_config:
                ...
                needed_VRAM = opt_offload * model_size / number_layers
            return [model_size, needed_VRAM, opt_offload]
        ...
        if self.model_config["model_type"] == "audio":
            if self.model_config["task"] == "transcribe" or self.model_config["task"] == "translate":
                ...
                model_size = float(self.read_model_config(config_path, self.model_config["model_name"]))
        ...
        if self.model_config["model_type"] == "vision":
            ...
            model_size = float(self.read_model_config(config_path, "model_size"))
            model_size_4bit = float(self.read_model_config(config_path, "model_size_4bit"))
    ...
    def get_available_VRAM():
        try:
            ...
            vram_gb = mem_info.total / (1024 ** 3)  # Convert bytes to GB
        except:
            vram_gb = 0
        return vram_gb
